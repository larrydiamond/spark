...
public abstract class AbstractBytesToBytesMapSuite {
  ...
  public void setup() {
    ...
    when(blockManager.getDiskWriter(
      ...
      any(ShuffleWriteMetrics.class))).thenAnswer(invocationOnMock -> {
        ...
        return new DiskBlockObjectWriter((File) args[1], serializerManager, (SerializerInstance) args[2], (Integer) args[3], false, (ShuffleWriteMetrics) args[4], (BlockId) args[0]);
          });
          }
          @After
  ...
  * Fast equality checking for byte arrays, since these comparisons are a bottleneck
    * in our stress tests.
    */
    private static boolean arrayEquals(byte[] expected, Object base, long offset, long actualLengthBytes) {
  ...
  countFreedPages++;
      ...
      if (destructive) {
        ...
        iteratorTestBase(true);
            ...
            final BytesToBytesMap map = new BytesToBytesMap(taskMemoryManager, NUM_ENTRIES, PAGE_SIZE_BYTES);
          try {
  ...
  Assert.assertEquals(key.length, loc.getKeyLength());
    ...
    for (Map.Entry<ByteBuffer, byte[]> entry : expected.entrySet()) {
      final byte[] key = JavaUtils.bufferToArray(entry.getKey());
        ...
        final BytesToBytesMap.Location loc = map.lookup(key, Platform.BYTE_ARRAY_OFFSET, key.length);
        Assert.assertTrue(loc.isDefined());
          Assert.assertTrue(arrayEquals(key, loc.getKeyBase(), loc.getKeyOffset(), loc.getKeyLength()));
          Assert.assertTrue(arrayEquals(value, loc.getValueBase(), loc.getValueOffset(), loc.getValueLength()));
          }
        } finally {
        map.free();
        }
    ...
    try {
      ...
      boolean success = true;
        ...
        if (i > 0) {
          ...
          BytesToBytesMap map = new BytesToBytesMap(taskMemoryManager, blockManager, serializerManager, 1, 0.75, 1024);
          try {
          int i;
            for (i = 0; i < 1024; i++) {
        final long[] arr = new long[] { i };
      ...
      for (File spillFile : spillFilesCreated) {
        assertFalse("Spill file " + spillFile.getPath() + " was not cleaned up", spillFile.exists());
          ...
          @Test
            ...
            BytesToBytesMap map = new BytesToBytesMap(taskMemoryManager, blockManager, serializerManager, 1, 0.5, 1024);
            try {
          int i;
          for (i = 0; i < 1024; i++) {
          final long[] arr = new long[] { i };
  ...
  Assert.fail("Expected IllegalArgumentException to be thrown");
    ...
    try {
      new BytesToBytesMap(taskMemoryManager, BytesToBytesMap.MAX_CAPACITY + 1, PAGE_SIZE_BYTES);
      Assert.fail("Expected IllegalArgumentException to be thrown");
        } catch (IllegalArgumentException e) {
      }
      try {
        new BytesToBytesMap(taskMemoryManager, 1, TaskMemoryManager.MAXIMUM_PAGE_SIZE_BYTES + 1);
    Assert.fail("Expected IllegalArgumentException to be thrown");
      } catch (IllegalArgumentException e) {
    }
  }
  ...
  map.free();
    ...
    memoryManager.limit(5000);
      BytesToBytesMap map = new BytesToBytesMap(taskMemoryManager, blockManager, serializerManager, 256, 0.5, 4000);
