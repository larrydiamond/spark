...
public class SparkSubmitCommandBuilderSuite extends BaseSuite {
  ...
  public void testCliParser() throws Exception {
    List<String> sparkSubmitArgs = Arrays.asList(parser.MASTER, "local", parser.DRIVER_MEMORY, "42g", parser.DRIVER_CLASS_PATH, "/driverCp", parser.DRIVER_JAVA_OPTIONS, "extraJavaOpt", parser.CONF, "spark.randomOption=foo", parser.CONF, SparkLauncher.DRIVER_EXTRA_LIBRARY_PATH + "=/driverLibPath", SparkLauncher.NO_RESOURCE);
      Map<String, String> env = new HashMap<>();
      List<String> cmd = buildCommand(sparkSubmitArgs, env);
      assertTrue(findInStringList(env.get(CommandBuilderUtils.getLibPathEnvName()), File.pathSeparator, "/driverLibPath"));
      ...
      assertTrue("Command should contain user-defined conf.", Collections.indexOfSubList(cmd, Arrays.asList(parser.CONF, "spark.randomOption=foo")) > 0);
      }
      @Test
      public void testShellCliParser() throws Exception {
      List<String> sparkSubmitArgs = Arrays.asList(parser.CLASS, "org.apache.spark.repl.Main", parser.MASTER, "foo", "--app-arg", "bar", "--app-switch", parser.FILES, "baz", parser.NAME, "appName");
    ...
    public void testAlternateSyntaxParsing() throws Exception {
        List<String> sparkSubmitArgs = Arrays.asList(parser.CLASS + "=org.my.Class", parser.MASTER + "=foo", parser.DEPLOY_MODE + "=bar", SparkLauncher.NO_RESOURCE);
  ...
  public void testPySparkLauncher() throws Exception {
    List<String> sparkSubmitArgs = Arrays.asList(SparkSubmitCommandBuilder.PYSPARK_SHELL, "--master=foo", "--deploy-mode=bar");
      Map<String, String> env = new HashMap<>();
      List<String> cmd = buildCommand(sparkSubmitArgs, env);
      assertTrue(Arrays.asList("python", "python2", "python3").contains(cmd.get(cmd.size() - 1)));
      assertEquals(String.format("\"%s\" \"foo\" \"%s\" \"bar\" \"%s\"", parser.MASTER, parser.DEPLOY_MODE, SparkSubmitCommandBuilder.PYSPARK_SHELL_RESOURCE), env.get("PYSPARK_SUBMIT_ARGS"));
      }
      @Test
  ...
  public void testSparkRShell() throws Exception {
    List<String> sparkSubmitArgs = Arrays.asList(SparkSubmitCommandBuilder.SPARKR_SHELL, "--master=foo", "--deploy-mode=bar", "--conf", "spark.r.shell.command=/usr/bin/R");
      Map<String, String> env = new HashMap<>();
      List<String> cmd = buildCommand(sparkSubmitArgs, env);
      assertEquals("/usr/bin/R", cmd.get(cmd.size() - 1));
      assertEquals(String.format("\"%s\" \"foo\" \"%s\" \"bar\" \"--conf\" \"spark.r.shell.command=/usr/bin/R\" \"%s\"", parser.MASTER, parser.DEPLOY_MODE, SparkSubmitCommandBuilder.SPARKR_SHELL_RESOURCE), env.get("SPARKR_SUBMIT_ARGS"));
  ...
  buildCommand(sparkSubmitArgs, env);
    ...
    public void testExamplesRunnerWithMasterNoMainClass() throws Exception {
      ...
      List<String> sparkSubmitArgs = Arrays.asList(SparkSubmitCommandBuilder.RUN_EXAMPLE, parser.MASTER + "=foo");
  Map<String, String> env = new HashMap<>();
  ...
  buildCommand(sparkSubmitArgs, env);
    ...
    @Test
      ...
      List<String> sparkSubmitArgs = Arrays.asList(SparkSubmitCommandBuilder.RUN_EXAMPLE, parser.MASTER + "=foo", parser.DEPLOY_MODE + "=bar", "SparkPi", "42");
      Map<String, String> env = new HashMap<>();
      List<String> cmd = buildCommand(sparkSubmitArgs, env);
    ...
    assertEquals(SparkSubmitCommandBuilder.EXAMPLE_CLASS_PREFIX + "SparkPi", findArgValue(cmd, parser.CLASS));
  ...
  new SparkSubmitCommandBuilder().buildSparkSubmitArgs();
    ...
    @Test
      ...
      assertTrue("By default application run in local mode", builder.isClientMode(Collections.emptyMap()));
  ...
  final String DRIVER_DEFAULT_PARAM = "-Ddriver-default";
    ...
    SparkSubmitCommandBuilder launcher = newCommandBuilder(Collections.emptyList());
  launcher.childEnv.put(CommandBuilderUtils.ENV_SPARK_HOME, System.getProperty("spark.test.home"));
  ...
  Map<String, String> env = new HashMap<>();
    List<String> cmd = launcher.buildCommand(env);
      ...
      assertTrue("Driver default options should be configured.", cmd.contains(DRIVER_DEFAULT_PARAM));
  ...
  assertFalse("Driver default options should not be configured.", cmd.contains(DRIVER_DEFAULT_PARAM));
  assertFalse("Driver extra options should not be configured.", cmd.contains(DRIVER_EXTRA_PARAM));
    }
  ...
  assertTrue("Driver classpath should contain provided entry.", contains("/driver", cp));
    ...
    assertTrue("Native library path should contain provided entry.", contains("/native", libPath.split(Pattern.quote(File.pathSeparator))));
    } else {
  ...
  assertEquals("yarn", findArgValue(cmd, parser.MASTER));
    ...
    assertTrue(opt + " should be contained in the final cmd.", cmd.contains(opt));
