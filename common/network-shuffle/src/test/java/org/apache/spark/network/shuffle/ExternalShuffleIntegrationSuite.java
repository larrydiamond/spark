...
public class ExternalShuffleIntegrationSuite {
  ...
  static byte[][] exec0Blocks = new byte[][] { new byte[123], new byte[12345], new byte[1234567] };
    static byte[][] exec1Blocks = new byte[][] { new byte[321], new byte[54321] };
    @BeforeClass
  ...
  rand.nextBytes(block);
    ...
    handler = new ExternalBlockHandler(new OneForOneStreamManager(), new ExternalShuffleBlockResolver(conf, null) {
    @Override
  ...
  try (ExternalBlockStoreClient client = new ExternalBlockStoreClient(clientConf, null, false, 5000)) {
    client.init(APP_ID);
  ...
  @Override
    ...
    requestsRemaining.release();
      ...
      registerExecutor("exec-0", dataContext0.createExecutorInfo(SORT_MANAGER));
    FetchResult exec0Fetch = fetchBlocks("exec-0", new String[] { "shuffle_0_0_0", "shuffle_0_0_1", "shuffle_0_0_2" });
    assertEquals(Sets.newHashSet("shuffle_0_0_0", "shuffle_0_0_1", "shuffle_0_0_2"), exec0Fetch.successBlocks);
  assertTrue(exec0Fetch.failedBlocks.isEmpty());
  ...
  assertBuffersEqual(new NioManagedBuffer(ByteBuffer.wrap(exec0RddBlockValid)), execFetch.buffers.get(0));
    }
  @Test
  ...
  public void testRemoveRddBlocks() throws Exception {
    ...
    client.init(APP_ID);
    Future<Integer> numRemovedBlocks = client.removeBlocks(TestUtils.getLocalHost(), server.getPort(), "exec-1", new String[] { validBlockIdToRemove, missingBlockIdToRemove });
  ...
  @Test
    ...
    registerExecutor("exec-0", dataContext0.createExecutorInfo(SORT_MANAGER));
    FetchResult execFetch = fetchBlocks("exec-0", new String[] { "shuffle_2_0_0" });
  ...
  assertEquals(Sets.newHashSet("shuffle_1_0_0"), execFetch1.failedBlocks);
    ...
    registerExecutor("exec-0", dataContext0.createExecutorInfo(SORT_MANAGER));
    FetchResult execFetch = fetchBlocks("exec-2", new String[] { "shuffle_0_0_0", "shuffle_1_0_0" });
  assertTrue(execFetch.successBlocks.isEmpty());
  ...
  assertEquals(Sets.newHashSet("shuffle_0_0_0", "shuffle_1_0_0"), execFetch.failedBlocks);
    ...
    public void testFetchNoServer() throws Exception {
      TransportConf clientConf = new TransportConf("shuffle", new MapConfigProvider(ImmutableMap.of("spark.shuffle.io.maxRetries", "0")));
    registerExecutor("exec-0", dataContext0.createExecutorInfo(SORT_MANAGER));
    FetchResult execFetch = fetchBlocks("exec-0", new String[] { "shuffle_1_0_0", "shuffle_1_0_1" }, clientConf, 1);
  assertTrue(execFetch.successBlocks.isEmpty());
